...
if [ -n "${TS_STAGED}" ]; then
  if [ -d "${REPO_DIR}/app/node_modules" ]; then
    ...
    ( cd "${REPO_DIR}/app" && npx --no-install tsc -b --pretty false ) > "${TSC_LOG}" 2>&1
    TSC_STATUS=$?
    tail -40 "${TSC_LOG}"
    rm -f "${TSC_LOG}"
...
if [ -n "${API_STAGED}" ]; then
  ...
  if [ ! -x "${VENV_PY}" ]; then
    ...
    if [ "${#TEST_TARGETS[@]}" -gt 0 ]; then
      ...
      if ( cd "${REPO_DIR}" && "${VENV_PY}" -m pytest -x -q --no-header ${UNIQ_TARGETS} ); then
        ok "pytest passed."
      else
...
if [ -n "${APP_STAGED}" ]; then
  if [ ! -d "${REPO_DIR}/app/node_modules" ]; then
    ...
    if [ "${#VITEST_TARGETS[@]}" -gt 0 ]; then
      ...
      # shellcheck disable=SC2086
      "${REPO_DIR}/scripts/run-vitest.sh" ${UNIQ_VITEST}
      VSTATUS=$?
      if [ "${VSTATUS}" = "0" ]; then
        ok "vitest passed."
      elif [ "${VSTATUS}" = "9" ]; then
