#!/bin/bash
# Test the yourOS installer in a fresh Ubuntu container.
#
# This script spins up a clean `ubuntu:22.04` container, copies the repo in,
# installs the system packages yourOS needs (no Homebrew), and runs ./install.sh.
...
CONTAINER_NAME="youros-install-test-$$"
...
echo -e "${BLUE}=== yourOS Linux install smoke test ===${NC}"
echo "Image: $IMAGE"
echo "Repo:  $REPO_DIR"
echo ""
...
docker run -d --name "$CONTAINER_NAME" \
    -v "$REPO_DIR":/workspace/youros:ro \
    "$IMAGE" \
    sleep 3600 > /dev/null
...
phase() {
    ...
    local cmd="$2"
    printf "  %-40s" "$name..."
    if run_in_container "$cmd" > /tmp/youros_phase.log 2>&1; then
        echo -e "${GREEN}PASS${NC}"
        return 0
    else
        echo -e "${RED}FAIL${NC}"
        echo "  --- last 20 lines of output ---"
        tail -20 /tmp/youros_phase.log | sed 's/^/    /'
        echo "  -------------------------------"
        return 1
    fi
}
...
phase "copy repo into writable dir" "cp -r /workspace/youros /root/youros && chmod +x /root/youros/install.sh" || FAILED=$((FAILED+1))
...
phase "python venv created" "cd /root/youros/api && python3 -m venv .venv && . .venv/bin/activate && pip install --quiet --upgrade pip" || FAILED=$((FAILED+1))

phase "api requirements installed" "cd /root/youros/api && . .venv/bin/activate && pip install --quiet -r requirements.txt" || FAILED=$((FAILED+1))

phase "frontend deps installed" "cd /root/youros/app && npm install --silent" || FAILED=$((FAILED+1))

phase "frontend builds" "cd /root/youros/app && npm run build" || FAILED=$((FAILED+1))
...
phase "install.sh parses as bash" "bash -n /root/youros/install.sh" || FAILED=$((FAILED+1))

echo ""
if [ "$FAILED" -eq 0 ]; then
    echo -e "${GREEN}All phases passed.${NC} yourOS installs cleanly on $IMAGE without Homebrew."
    exit 0
else
