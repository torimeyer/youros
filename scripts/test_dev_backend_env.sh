...
# ~/.myos/.env (if it exists) before launching uvicorn, and that
# they do NOT error when ~/.myos/.env is absent.
#
...
# --- Setup: temp HOME so we never touch the real ~/.myos/.env ---
ORIG_HOME="$HOME"
TMP_HOME=$(mktemp -d)
...
run_env_block() {
    ...
    eval "$(grep -A5 'if \[ -f "\$HOME/.myos/.env" \]' "$script" | head -6)"
}

# -----------------------------------------------------------------------
# Test 1: start.sh — env-load block is present
# -----------------------------------------------------------------------
if grep -q 'if \[ -f "\$HOME/.myos/.env" \]' "$REPO_DIR/start.sh"; then
...
# -----------------------------------------------------------------------
if grep -A5 'if \[ -f "\$HOME/.myos/.env" \]' "$REPO_DIR/start.sh" | grep -q 'set -a'; then
...
# -----------------------------------------------------------------------
if grep -q 'if \[ -f "\$HOME/.myos/.env" \]' "$SCRIPT_DIR/dev-backend.sh"; then
...
# -----------------------------------------------------------------------
if grep -A5 'if \[ -f "\$HOME/.myos/.env" \]' "$SCRIPT_DIR/dev-backend.sh" | grep -q 'set -a'; then
...
# -----------------------------------------------------------------------
# Test 5: no error when ~/.myos/.env does NOT exist
# -----------------------------------------------------------------------
# Ensure the file is absent in our temp HOME.
rm -f "$TMP_HOME/.myos/.env"
...
(
    ...
    echo "no_error"
) | grep -q "no_error" && pass "env-load block exits cleanly when ~/.myos/.env is absent" \
    || fail "env-load block errored when ~/.myos/.env is absent"

# -----------------------------------------------------------------------
# Test 6: exported var is visible after sourcing a real .env file
# -----------------------------------------------------------------------
mkdir -p "$TMP_HOME/.myos"
echo "MYOS_ENV_TEST=hello" > "$TMP_HOME/.myos/.env"
...
if [ "$result" = "hello" ]; then
    pass "MYOS_ENV_TEST=hello correctly exported after sourcing ~/.myos/.env"
else
