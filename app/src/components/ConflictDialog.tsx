...
export default function ConflictDialog({ open, conflicts, onWait, onProceed }: Props) {
  ...
  return (
    ...
    >
      <div
        className="bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl w-full max-w-lg mx-4 flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div
          data-testid="conflict-dialog-header"
          className="flex items-center gap-2 px-5 py-4 border-b border-slate-800"
        >
        ...
        <div className="px-5 py-4 max-h-72 overflow-y-auto space-y-2">
          {conflicts.map((c, i) => (
            <div
              ...
              className="flex items-start gap-3 px-3 py-2 rounded-lg bg-slate-800/40 border border-slate-800"
            >
              <Icon name="lock" className="text-amber-400/80 text-sm shrink-0 mt-0.5" />
        ...
        <div className="flex items-center justify-end gap-2 px-5 py-3 border-t border-slate-800 bg-slate-900/60">
          <button
            ...
            onClick={onWait}
            className="text-sm px-4 py-2 rounded-lg text-slate-300 hover:bg-slate-800 transition-colors"
          >
            Wait
          </button>
          <button
