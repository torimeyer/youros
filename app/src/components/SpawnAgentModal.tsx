...
export function SpawnAgentModal({ path, title, checks, onClose, onSpawned }: SpawnAgentModalProps) {
  ...
  async function handleSpawn() {
    ...
    setError(null)
    try {
      const result = await buildSpec(path, undefined, model)
      if (result.status === 'ok') {
  ...
  const confirmLabel = loading ? 'Spawning…' : `Spawn ${model === 'claude' ? 'Claude' : 'Gemini'} agent`

  return (
    <div
      data-testid="spawn-agent-modal"
    ...
    >
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-6 max-w-md w-full mx-4 shadow-xl">
        <h2 className="text-white text-lg font-semibold mb-1">Spawn agent</h2>
