...
export function IMessageConfirmBubble({
  ...
  if (resolving) {
    return (
      <div
        ...
        className="inline-flex items-center gap-2 px-4 py-3 rounded-xl border border-slate-600/40 bg-slate-800/40 text-sm text-slate-400 max-w-[85%]"
      >
  ...
  return (
    <div
      ...
      className="inline-flex flex-col gap-3 px-4 py-3 rounded-xl border border-blue-500/30 bg-blue-500/5 text-sm text-slate-300 max-w-[85%]"
    >
      <div>
        <span className="text-xs text-slate-400 block mb-1">Send via iMessage?</span>
        <span className="font-medium text-blue-200">{displayName}</span>
        {preview && (
          <p className="text-slate-400 text-xs mt-1 italic">&ldquo;{preview}&rdquo;</p>
        )}
      </div>
