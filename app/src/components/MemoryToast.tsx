  ...
  return (
    ...
    >
      ...
      <div className="min-w-0">
        ...
        <p className="text-xs text-slate-400 mt-0.5 truncate" title={bullet}>
          {bullet}
        </p>
      </div>
      <button
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="text-sm font-medium text-slate-100 leading-snug">Forgot.</p>
        <p className="text-xs text-slate-400 mt-0.5 truncate" title={forgot}>
          {forgot}
        </p>
      </div>
      <button
        ...
        className="shrink-0 text-xs font-medium text-violet-400 hover:text-violet-300 transition-colors px-2 py-1 rounded border border-violet-700 hover:border-violet-500"
      >
        Undo
      </button>
      <button
...
function AmbiguousToast({
  ...
  return (
    <div
      ...
      className="fixed bottom-6 right-6 z-50 flex flex-col gap-2 rounded-xl bg-slate-800 border border-slate-700 px-4 py-3 shadow-xl max-w-sm animate-fade-in"
    >
      <div className="flex items-center justify-between">
        <p className="text-sm font-medium text-slate-100">Which one to forget?</p>
        <button
          onClick={onDismiss}
          aria-label="Dismiss"
      ...
      <ul className="flex flex-col gap-1">
        {candidates.map((c) => (
          <li key={c}>
            <button
              className="w-full text-left text-xs text-slate-300 hover:text-white px-2 py-1 rounded hover:bg-slate-700 transition-colors"
              onClick={onDismiss}
            >
              {c}
            </button>
