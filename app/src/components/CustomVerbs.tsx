        ...
        <ul className="space-y-2 mb-4" data-testid="custom-verbs-list">
          {verbs.map((verb) => (
            <li
              ...
              className="flex items-start gap-3 px-3 py-2.5 bg-slate-100 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700/50"
            >
              <code className="text-sm font-mono text-accent-400 flex-shrink-0 pt-px">{verb.name}</code>
              <span className="flex-1 text-sm text-slate-700 dark:text-slate-300">{verb.description}</span>
              <button
                data-testid={`remove-verb-${verb.name}`}
                onClick={() => handleRemove(verb.name)}
                disabled={removing === verb.name}
                aria-label={`Remove ${verb.name}`}
                className="flex-shrink-0 text-slate-500 hover:text-red-600 dark:hover:text-red-400 transition-colors disabled:opacity-40 text-base leading-none"
              >
                ×
              </button>
            </li>
        ...
        <div className="flex gap-2">
          <div className="flex-1">
            <input
              ...
              className="w-full px-3 py-2 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-slate-800 dark:text-slate-200 placeholder-slate-500 focus:outline-none focus:border-slate-500"
            />
            {nameError && (
              <p data-testid="verb-name-error" className="text-xs text-red-600 dark:text-red-400 mt-1">{nameError}</p>
            )}
          </div>
          <input
            ...
            className="flex-1 px-3 py-2 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-slate-800 dark:text-slate-200 placeholder-slate-500 focus:outline-none focus:border-slate-500"
          />
          <button
