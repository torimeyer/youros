      ...
      <div className="flex gap-2">
        {TURN_OPTIONS.map(n => (
          <button
            ...
            onClick={() => onPick(n)}
            className="px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-blue-500/20 border border-slate-200 dark:border-slate-700 hover:border-blue-500/50 text-slate-700 dark:text-slate-300 hover:text-blue-200 text-xs font-medium transition-colors"
          >
            {n} {n === 1 ? 'turn' : 'turns'}
          </button>
        ))}
