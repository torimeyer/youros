...
// Classic palette: white on black.
const CLR_BG = '#000000'
const CLR_FG = '#ffffff'
...
export default function Pong() {
  ...
  useEffect(() => {
    ...
    let raf = 0
    function frame() {
      // Pause physics while a dialog/how-to-play overlay is open.
      if (!s.paused && !isOverlayOpen()) {
      ...
      if (ctx) {
        ...
        ctx.fillStyle = CLR_FG
        ctx.font = 'bold 40px monospace'
        ctx.textAlign = 'right'
        ctx.fillText(String(sc.you), W / 2 - 20, 50)
        ctx.textAlign = 'left'
        ctx.fillText(String(sc.cpu), W / 2 + 20, 50)
  ...
  return (
    <div ref={containerRef} className="flex w-full flex-col items-center gap-4">
      <div className="flex w-full flex-col gap-1">
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Pong</h2>
        <p className="text-xs text-slate-500 dark:text-slate-400">
          Move with the arrow keys (or your mouse). First to {WIN} wins. You: {score.you} · Computer: {score.cpu}
        </p>
      </div>

      <canvas ref={canvasRef} data-testid="pong-canvas" className="max-w-full rounded border border-slate-300 dark:border-slate-700" />
