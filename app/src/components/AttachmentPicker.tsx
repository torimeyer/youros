...
function FileRow({
  ...
  return (
    <button
      ...
      data-testid={testId}
      onClick={onClick}
      className="w-full flex items-center gap-3 px-4 py-3 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors text-left group"
    >
      <div className="w-8 h-8 rounded bg-slate-200 dark:bg-slate-700 flex items-center justify-center shrink-0 text-slate-600 dark:text-slate-400 text-xs font-mono uppercase">
        {name.split('.').pop()?.slice(0, 3) ?? '?'}
      </div>
      <div className="flex-1 min-w-0">
        <div className="text-sm text-white truncate group-hover:text-pink-700 dark:hover:text-pink-300 transition-colors">
          {name}
        </div>
  ...
  return (
    ...
    >
      <div
        ...
        className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl w-full max-w-lg flex flex-col shadow-2xl"
      ...
      >
        ...
        {/* Tabs */}
        <div className="flex gap-0 px-4 pt-3 pb-0 border-b border-slate-200 dark:border-slate-800 shrink-0">
