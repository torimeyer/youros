  ...
  return (
    <>
      <button
        ...
        className="text-xs text-slate-400 hover:text-amber-400 border border-slate-700 hover:border-amber-500 rounded px-2.5 py-1 transition-colors disabled:opacity-50"
      >
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-xl w-full max-w-2xl shadow-2xl flex flex-col max-h-[80vh]">
            <div className="flex items-center justify-between p-5 border-b border-slate-800">
              <div>
                ...
                {change && (
                  <p className="text-slate-400 text-xs mt-0.5 font-mono">{change.message}</p>
                )}
              </div>
              <button
            ...
            {phase === "done" ? (
              <div className="p-5 text-center">
                <p data-testid="undo-success-msg" className="text-green-400 font-medium">
                  Done. The change has been undone.
                </p>
                <p className="text-slate-500 text-xs mt-1">A new "undo" commit was added to git.</p>
                <button
                  onClick={handleClose}
                  className="mt-4 px-4 py-2 text-sm bg-slate-800 hover:bg-slate-700 text-white rounded-lg transition-colors"
                >
                  Close
                </button>
              </div>
              ...
              <div className="p-5 text-center">
                <p data-testid="undo-error-msg" className="text-red-400 text-sm">{errorMsg}</p>
                <button
                  onClick={handleClose}
                  className="mt-4 px-4 py-2 text-sm bg-slate-800 hover:bg-slate-700 text-white rounded-lg transition-colors"
                >
              ...
              <>
                  ...
                  <>
                    <div className="px-5 pt-4 pb-2">
                      <p className="text-xs text-slate-400 mb-1">
                        Files affected: <span className="text-slate-300">{change.files_changed.join(", ") || "none"}</span>
                      </p>
                    </div>
                    <div
                      data-testid="undo-diff-view"
                      className="flex-1 overflow-auto mx-5 mb-4 bg-slate-950 border border-slate-800 rounded-lg p-3 font-mono text-xs text-slate-300 whitespace-pre"
                    >
                      {change.diff || "(no diff)"}
                    </div>
                  </>
                ...
                <div className="flex items-center justify-end gap-2 p-4 border-t border-slate-800">
                  <button
                    ...
                    className="px-4 py-2 text-slate-400 hover:text-white text-sm rounded-lg transition-colors disabled:opacity-50"
                  >
                    Cancel
                  </button>
                  <button
