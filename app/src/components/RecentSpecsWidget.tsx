...
export default function RecentSpecsWidget() {
  ...
  const handleSpecClick = (specPath: string) => {
    navigate(`/specs?highlight=${encodeURIComponent(specPath)}`);
  };
          ...
          <div className="space-y-2">
            {specs.map((spec) => {
              ...
              return (
                <button
                  key={spec.path}
                  onClick={() => handleSpecClick(spec.path)}
                ...
                >
                  <div className="w-8 h-8 rounded-full bg-purple-500/20 flex items-center justify-center shrink-0 mt-0.5">
                    <Icon name="description" className="text-purple-400" size={16} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-white truncate">{spec.title}</p>
