...
function Countdown({ wakeupAt }: { wakeupAt: number | null }) {
  ...
  if (secs === null) return <span className="text-slate-400">Monitoring&hellip;</span>

  const m = Math.floor(secs / 60)
  const s = secs % 60
  const label = m > 0 ? `${m}m ${s}s` : `${s}s`
  return <span className="text-slate-300 tabular-nums">{label}</span>
}
...
function SinglePill({
  ...
  return (
    <div
      ...
      className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-slate-800 border border-slate-700 text-xs"
    >
      <span className="text-slate-400 truncate max-w-[120px]" title={task.label}>
        {task.label}
      </span>
...
export default function ParkedTaskPill({ tasks, onCancelled }: ParkedTaskPillProps) {
  ...
  return (
    <div
      ...
      className="flex items-center gap-1.5 px-4 py-1.5 border-b border-slate-800/60 flex-wrap"
    >
      {visible.map(t => (
      ...
      {overflow > 0 && (
        <div className="relative">
          <button
            ...
            className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-slate-800 border border-slate-700 text-xs text-slate-400 hover:text-white transition-colors"
          >
            +{overflow} more
          </button>
          {popoverOpen && (
            <div
              ...
              className="absolute top-full left-0 mt-1 z-50 bg-slate-900 border border-slate-700 rounded-lg shadow-xl p-2 flex flex-col gap-1.5 min-w-[200px]"
            >
