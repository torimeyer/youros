    ...
    <div className="relative w-full" ref={ref}>
      ...
      {open && (
        <div
          ...
          className="absolute right-0 mt-2 w-44 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg z-20"
        >
          {formats.map((f) => (
            <button
              key={f}
              ...
              className="w-full text-left px-3 py-2 text-sm text-slate-800 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 first:rounded-t-lg last:rounded-b-lg"
            >
              {FORMAT_LABELS[f]}
            </button>
          ))}
