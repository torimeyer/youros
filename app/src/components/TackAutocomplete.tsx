...
const TYPE_COLORS: Record<Suggestion['type'], string> = {
  ...
  query: 'text-purple-400 bg-purple-500/10',
  needle: 'text-orange-400 bg-orange-500/10',
  focus: 'text-pink-400 bg-pink-500/10',
}
...
export default function TackAutocomplete({ inputValue, onSelect, keyHandlerRef }: TackAutocompleteProps): React.ReactElement {
  ...
  return (
    <div
      className="absolute bottom-full left-0 right-0 mb-1 bg-slate-900 border border-slate-700 rounded-lg shadow-lg overflow-hidden z-50"
    ...
    >
      {suggestions.map((s, i) => (
        <button
          ...
          className={`w-full text-left px-3 py-1.5 text-sm flex items-center gap-2 transition-colors ${i === activeIndex ? 'bg-slate-800' : 'hover:bg-slate-800/50'}`}
