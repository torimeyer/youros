...
@pytest.mark.skip(reason="→1270: ostk 6.0.5 binary uses stale cached register for [loadavg]; upstream fix needed before this test can run")
def test_loadavg_line_present_and_nonzero():
    ...
    reading live disk correctly. The binary fix is tracked upstream in →1270.
    """
...
def test_loadavg_fleet_nonzero():
    ...
    Upstream binary fix tracked in →1270.
    """
    with tempfile.TemporaryDirectory() as tmpdir:
        ostk_dir = Path(tmpdir) / ".ostk"
