...
from api.routers.gemini import _detect_gemini, is_gemini_available
...
async def test_is_gemini_available_false_when_unavailable():
    with patch("api.routers.gemini._detect_gemini", new=AsyncMock(return_value={
        "available": False,
        "authenticated": False,
        "email": None,
        ...
        with patch("api.routers.gemini._gemini_status_cache", None):
...
async def test_gemini_status_endpoint_returns_dict():
    import importlib
    import api.routers.gemini as mod
