...
hardcoded "myOS" used as a role label) do not sneak back into source code.

The product name "myOS" is allowed in product-description text.  What is
NOT allowed is:
  - "Loading torios" (a specific user's instance name baked into a loading
    screen instead of reading instanceName from the store)
  - Using a raw string literal "myOS" as the role label in a chat/transcript
    component (e.g. `role === "user" ? "You" : "myOS"`) instead of the
    store value
...
class TestNoHardcodedInstanceName:
    ...
    def test_no_hardcoded_role_label_myos(self):
        """The chat/transcript role label must not be the raw string 'myOS'.
        It must read from the instanceName store.

        Specifically this catches the pattern:
            {msg.role === "user" ? "You" : "myOS"}
        which was the bug in Transcripts.tsx before commit 2.
        """
        src = _frontend_src()
        if not src.exists():
            return
        ...
        # Pattern: colon followed by "myOS" in double quotes (ternary role label).
        # This matches: : "myOS"  (role fallback in JSX)
        pattern = re.compile(r':\s*"myOS"')
        hits = _scan(files, pattern)

        assert hits == [], (
            "Found hardcoded '\"myOS\"' used as a role label in frontend source. "
            "Use instanceName from the app store instead.\n"
            + "\n".join("  {}:{}: {}".format(p, n, l) for p, n, l in hits)
        )
    ...
    def test_no_torios_as_instance_name_default_in_services(self):
        ...
        instance_name.  The only correct default is 'myOS'.
        ...
        assert hits == [], (
            ...
            "instance_name default. Use 'myOS' as the default fallback.\n"
            + "\n".join("  {}:{}: {}".format(p, n, l) for p, n, l in hits)
        )
