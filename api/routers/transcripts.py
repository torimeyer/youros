...
# →2894 family 6: bare speaker labels ("Assistant:", "User:", "Human:", …)
# that appear as the entire content of a message after template stripping.
# These are never meaningful task prose and should never become titles.
...
async def _generate_titles_background(items: list[tuple[str, Path]]) -> None:
    ...
    for session_id, jsonl_path in items:
        ...
        cached_is_junk = cached is not None and (
            _looks_like_refusal(cached) or _looks_like_instruction_paraphrase(cached)
        )
        ...
        with open(jsonl_path, "r") as f:
            for line in f:
                ...
                if entry.get("type") == "user" and "message" in entry:
                    ...
                    if not cleaned:
                        continue
                    # A bare speaker label ("Assistant:", "User:", …) is
                    # never task prose; the template-stripped content was
                    # empty (→2894 family 6).
                    if _BARE_SPEAKER_LABEL_RE.fullmatch(cleaned.strip()):
                        continue
...
async def backfill_transcript_titles():
    ...
    # →2894 families: brief rule sentence titles (family 5), bare speaker
    # label titles (family 6), and AI paraphrases of the instruction sheet
    # (family 7).
    brief_rule_title_re = re.compile(r"(?i)^exiting\s+after\s+analysis\s+only")
    bare_speaker_label_re = re.compile(r"(?i)^(assistant|user|human|ai|claude)\s*:?\s*$")
    ...
    for session_id, title in list(cache.items()):
        if (
            ...
            or bare_speaker_label_re.fullmatch(title)
            or _looks_like_instruction_paraphrase(title)
        ):
