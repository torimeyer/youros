...
torios runs on your computer. It is not a cloud service. There is no
torios-operated backend that receives your data.

## What torios stores, and where

Everything torios keeps about you lives in two places on your own disk:

* `~/.myos/` - tasks, notes, chat history, agent transcripts,
  preferences, and any files you drop into torios.
* The torios application folder - the program itself, never your
  personal content.

Nothing in those folders is sent to torios. If you delete `~/.myos/`,
your torios data is gone.

## What leaves your computer

torios talks to the internet only in the ways you choose to turn on.
Each one is optional, and each one goes directly from your machine to
the third party. torios is not in the middle.
...
* **GitHub, Slack, and other integrations** - same pattern. Only
  active if you connect them. torios does not receive a copy.
* **Update checks** - torios checks GitHub for new releases. Only the
  release metadata is fetched. Nothing about you is sent.

There is no telemetry. torios does not phone home. We do not collect
...
* **Delete** - remove `~/.myos/` and disconnect any integrations in
  ...
  revoke the API keys you issued to torios.
* **Move to another computer** - copy `~/.myos/` across. torios will
  pick it up on launch.

## Questions

torios is open source. If you want to verify any of the above, read
