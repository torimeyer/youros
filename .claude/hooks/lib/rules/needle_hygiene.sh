...
_needle_hygiene_check() {
  local tool="$1" TITLE="$2" PRIORITY="$3" AC="$4"
  local min_len
  min_len=$(rule_param "needle_hygiene.min_title_length" "5")

  # ---- 1. Garbage title check ----
  if [ -z "$TITLE" ]; then
    log_rule_fire "needle_hygiene" "$tool" "block" "empty title"
    deny "title is empty. Every needle needs a clear, specific title that describes the work."
  fi

  local TITLE_LEN=${#TITLE}
  if [ "$TITLE_LEN" -lt "${min_len:-5}" ]; then
    log_rule_fire "needle_hygiene" "$tool" "block" "title too short (${TITLE_LEN} chars)"
  ...
  garbage_prefixes=$(rule_param "needle_hygiene.garbage_prefixes" '["session in","untitled","fix it","todo","test"]')

  for pattern in "session in" "untitled" "fix it" "todo" "test"; do
    if echo "$TITLE_LOWER" | grep -qi "^${pattern}"; then
      log_rule_fire "needle_hygiene" "$tool" "block" "garbage prefix in title"
  ...
  if [ "$WORD_COUNT" -le 1 ]; then
    for word in "sort" "fix" "update" "test" "todo" "done" "work" "task" "thing" "stuff" "misc" "other" "temp" "wip"; do
      if [ "$TITLE_LOWER" = "$word" ]; then
        log_rule_fire "needle_hygiene" "$tool" "block" "single generic word title: $word"
  ...
  if [ -n "$PRIORITY" ]; then
    case "$PRIORITY" in
      p0|p1|p2|p3)
        ...
        log_rule_fire "needle_hygiene" "$tool" "block" "lowercase priority $PRIORITY"
  ...
  if [ "$PRIORITY" = "P0" ] || [ "$PRIORITY" = "P1" ]; then
    if [ -z "$AC" ]; then
      log_rule_fire "needle_hygiene" "$tool" "block" "P0/P1 without AC"
  ...
  log_rule_fire "needle_hygiene" "$tool" "allow" "title and priority valid"
  return 0
}
