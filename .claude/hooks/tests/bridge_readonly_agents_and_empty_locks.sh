...
# the hook should try to POST and fail with exit 2 from the
# "backend unreachable" path, NOT the "missing locks" path.
...
RC=$?
if [ "$RC" -ne 2 ]; then
    echo "FAIL [empty Locks: [] accepted]: expected exit 2, got $RC" >&2
    FAILS=$((FAILS + 1))
elif grep -q "did not declare Locks" "$STDERR_FILE"; then
    echo "FAIL [empty Locks: [] accepted]: hook rejected empty locks as missing" >&2
    cat "$STDERR_FILE" >&2
    FAILS=$((FAILS + 1))
elif grep -q "unreachable\|connect-timeout\|refused" "$STDERR_FILE"; then
    echo "  ok: empty Locks: [] accepted (reached backend-unreachable, not missing-locks)"
else
    echo "  ok: empty Locks: [] accepted (exit 2, not missing-locks error)"
fi
...
RC=$?
if [ "$RC" -ne 2 ]; then
    echo "FAIL [Locks: [path] accepted]: expected exit 2, got $RC" >&2
    FAILS=$((FAILS + 1))
elif grep -q "did not declare Locks" "$STDERR_FILE"; then
    echo "FAIL [Locks: [path] accepted]: hook rejected valid locks" >&2
    FAILS=$((FAILS + 1))
else
    echo "  ok: Locks: [api/auth.py] accepted"
fi
