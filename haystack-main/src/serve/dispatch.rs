...
const OSTK_INSTRUCTIONS: &str = r#"ostk — coordination kernel. Use shell/spawn/interact instead of Bash. Output is compressed, file state is tracked, conflicts resolve automatically.

## Shell (shell — replaces Bash)
  shell(cmd="cargo test")                        — run command, get compressed output
  shell(cmd="cat src/main.rs")                   — read a file
  shell(cmd="grep -rn 'pattern' src/")           — search
  shell(cmd="git diff HEAD~1")                   — any shell command
...
## Boot protocol (do this first, every session)
  shell(cmd="ostk boot")                         — read .ostk/boot.md, report state
