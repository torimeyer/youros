...
The quickest way I put it is that a spec is a PRD for agents. A normal PRD is written for people, and people fill in the gaps from context and a hallway conversation, but an agent does exactly what's on the page, so writing for an agent is different in a few ways, the acceptance criteria have to be concrete and checkable instead of adjectives like "make it feel premium", the non-goals and constraints have to be written down instead of assumed, and the whole thing has to be self-contained because the agent wasn't in the room. On top of that the spec is executable, each criterion spawns a builder and the spec marks itself done when they all land, an agent can't start until I mark it Ready, and it stays in sync with the work instead of going stale. So a PRD is a spec for humans, and a spec here is a PRD for agents, written precisely enough that something non-human can build straight from it.
