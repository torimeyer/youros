# Connect Jira and Confluence to yourOS
...
## What yourOS can do with Jira and Confluence

Once connected, yourOS can:
...
1. Go to **Settings** in yourOS and find the **Atlassian** section.
...
**Why:** In May 2026 yourOS updated the permissions it requests during sign-in. Atlassian deprecated the old permission names in 2024, and their newer API endpoints now reject tokens that only have the old ones. Tokens issued before this update are missing the new permissions, so Jira and Confluence calls will start returning 401 (access denied) errors.
...
yourOS previously requested broad permission names like `read:jira-work` and `write:jira-work`. Those names still work in older Atlassian APIs but are deprecated and rejected by newer v2 endpoints.

yourOS now requests the granular equivalents alongside the legacy ones during the consent step:
...
yourOS also requests `offline_access`, which is what lets Atlassian issue a refresh token. Without it, your token would expire every hour and yourOS would have to ask you to sign in again constantly.

---

## How yourOS detects a stale or expired token

The `/api/atlassian/status` endpoint now includes an `expired` field in its response. yourOS checks this when you open the Atlassian section and when the connection card loads.
...
When `expired` is `true`, yourOS shows a **reconnect banner** on the Atlassian card before any Jira or Confluence call is made. This means you get a clear prompt to reconnect instead of seeing a confusing error in the middle of a task.

Under the hood, the status check makes a lightweight call to the Atlassian API to verify your token is still valid. If that call fails, `expired` is set to `true`. If yourOS cannot reach Atlassian at all (e.g., you are offline), it leaves `expired` as `false` so the banner does not appear incorrectly.
...
Your Atlassian administrator may have restricted third-party OAuth apps. Ask them to allow apps requesting the permissions listed in the table above, or to approve `yourOS` in the Atlassian admin panel.

**I connected but yourOS shows no Jira issues.**
Make sure you have issues assigned to you that are not in a "Done" state. yourOS only shows open, assigned issues by default.
